...
public:
  ...
  void run(const std::vector<std::string>& filePath)
  {
    _setup(filePath);
...
private:
  ...
  void _setup(const std::vector<std::string>& filePaths)
  {
    ...
    else
    {
      ...
      {
        _pipeline.registerContent(path, std::make_shared<player::content::VideoContent>(path));
      }
    }
  }
